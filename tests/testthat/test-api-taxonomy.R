...
test_that("list_terminal_descendants is a flag", {
    skip_on_cran()
    expect_error(.taxonomy_taxon_info(ott_id = 515698, list_terminal_descendants = c(TRUE, FALSE)),
                 "is not a flag")
    expect_error(.taxonomy_taxon_info(ott_id = 515698, list_terminal_descendants = c("na")),
                 "is not a flag")
    expect_error(.taxonomy_taxon_info(ott_id = 515698, list_terminal_descendants = c(1235)),
                 "is not a flag")
})
...
test_that("ott_id is not null for .taxonomy_mrca", {
    skip_on_cran()
    expect_error(.taxonomy_mrca(NULL),
                 "Must supply")
})

test_that("ott_id is a numeric for .taxonomy_mrca", {
    skip_on_cran()
